...
\noindent\textit{(Continued on next page.)}
\newpage
...
\noindent\textit{(Explanation of copyleft clause detection on next page.)}
\newpage
...
\begin{itemize}
	...
	This has a variant, namely \textbf{Binary delivery of Linked work With Header files OF Library Included In Linked work}, which exists to suit the more specific requirements of particular licenses it is used in.
...
This construction is a construction that the OSADL has already examined for a lot of the licenses it covers. It provides the raw data of these analyses, which are still the result of human curation, as a matrix which we can examine. In this matrix, we have the following types of results:
...
Given these possible classifications, it is useful to tie this into the OSADL's definitions of compatibility and incompatibility. Shown below are both definitions, copied verbatim from the dataset provided by the OSADL~\cite{osadl-license-checklists-scope}.
...
It should be noted that the OSADL presents these clauses as an unordered list. For our purposes however, we numbered these lists such that we can refer to C1, C2 and C3 as the compatibility cases, and I1, I2 and I3 as the incompatibility cases. We will use these defined names to more easily determine two interesting properties defined in the bullet list below. We need these properties to ensure we are working in a coherent system where assessments by an automated process of a given license combination is either compatible, or incompatible, but not both or neither. If we establish this as a fact, it is warranted to limit our automated process presented in Chapter~\ref{ch:method} to one answer, being exactly one category, per license combination it is instructed to assess.\\

\textbf{(In)compatibility definition properties:}

\begin{enumerate}
	\item \textbf{Completeness property:} All license combinations are covered by these definitions. 
	\item \textbf{Mutual exclusion property:} A given license combination belongs in at most one definition, either Compatibility or Incompatibility.
\end{enumerate}

We prove the \textbf{completeness property} by presenting a logical induction. Firstly, we use the presence of a copyleft clause. On this, it can be stated that a license either has one, or not. This is a property that holds true for any given license. With this property, we have enough information to classify each category verbosely, presenting a combination as LeadingLicenseCategory-CopyleftLicenseCategory:
...
Now we analyze the \textbf{mutual exclusion property}, and provide a theoretical example as to why both definitions can apply at the same time. Consider the following licenses:
...
\textbf{Theoretical subordinate license:}
...
Bringing these assessments together, we still say that it is warranted for this thesis to allow our automated process to assess each license combination, and say it is compatible or incompatible as a mutually exclusive set. We do this despite the absence of the \textbf{mutual exclusion property}, for which we reason that violations of this property must be specifically crafted and do not exist in practice.
...
If we make this spreadsheet (which the reproduction package can automatically generate) with one row for each combination, and add two columns indicating the OSADL-provided copyleft license table entry for the leading and the subordinate license, we can already make some important distinctions. For each category, we also list the number of combinations out of the total number of combinations that belong to this category in the dataset.
...
In this Chapter, we examined the work laid out by the OSADL. The starting point of all information they provide is the open source license compliance checklists, which provide pseudocode-like interpretations of what measures need to be taken in the general case to be compliant with using a given license in a particular way.

We also provided an example of how one detects the important property of copyleft clause presence based on a given checklist. In these checklists, that is mentioned explicitly, but it can also implicitly be constructed from the checklist statements itself, as we have done with the GPL-1.0-only license as an example. \\

With these facts established, we lay the ground work for proposing a method that automatically assesses a license combination, built on top of the following key observations:

\begin{itemize}
	\item The dataset is relatively complete, by that we mean to say only 2 licenses out of the 116 have been assessed as questionable when it comes to those licenses having a copyleft license.
	\item Similarly, only 1020 of the total 13456 possible combinations of said licenses was assessed to be unknown when it comes to the compatibility of that license combination.
	\item When we examined the remaining license combinations inside the context of several classes, the presence of a copyleft clause in a license became a very powerful indicator for assessing combinations with that license.
	\item We assessed that it is permissible to allow an automated system to answer yes or no, and not neither or both, to the question: Does the given license contain a copyleft clause?
\end{itemize}

The third observation already forms a part of the answer to our first research question. Given these observations combined, the next chapter will propose a method for automatically assessing license combinations, providing the remainder of the answers to the research questions we proposed in Chapter~\ref{ch:introduction}.