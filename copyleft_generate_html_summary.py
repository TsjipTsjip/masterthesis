...
def sort_filenames(filenames):
    """
    Sorts filenames based on the run number, which is not leading 0, so normal string sorting fails.
    Format: {LLM_NAME}-{RUN_NUMBER}_copyleft.json
    LLM_NAME can contain dashes.
    """
    def extract_run_number(filename):
        base = filename.split('/')[-1]  # Remove path
        parts = base.split('-')
        run_part = parts[-1]  # Get the last part which contains run number and suffix
        run_number_str = run_part.split('_')[0]  # Extract run number before '_copyleft.json'
        try:
            return int(run_number_str)
        except ValueError:
            return float('inf')  # If no valid number, put it at the end

    return sorted(filenames, key=extract_run_number)  # Returns a new list.


def get_shared_llm_name(filanems):
    """
    Extracts the shared LLM name for a list of filenames, returning None if they differ.
    """
    llm_names = set()
    for filename in filanems:
        base = filename.split('/')[-1]  # Remove path
        parts = base.split('-')
        llm_name = '-'.join(parts[:-1])  # All parts except the last one
        llm_names.add(llm_name)

    if len(llm_names) == 1:
        return llm_names.pop()
    else:
        return None
...
if __name__ == "__main__":
    ...
    result_filenames = sort_filenames(result_filenames)
    llm_name = get_shared_llm_name(result_filenames)
    ...
    generate_html_summary(results, f"copyleft_summary_{llm_name}.html" if llm_name else "copyleft_summary.html")