...
def generate_license_fulltext_download_script(license_ids, script_filename="spdx/download_fulltext.sh"):
    ...
    with open(script_filename, 'w', encoding='utf-8') as script_file:
        ...
        for license_id in license_ids:
            # When a license ID contains the 'WITH' keyword, that means a concatenation in that order of the adjoining ID's.
            # Sometimes this is wrapped with spaces, sometimes it is wrapped with dashes. Whatever the case, strip it off.
            parts = [el.strip(" -") for el in license_id.split("WITH")]
            output_redirector = ">"
            for part in parts:
                # URL-encode the license ID to handle special characters
                part_encoded = urllib.parse.quote(part)
                download_url = LICENSE_FULLTEXT_DOWNLOAD_TEMPLATE.format(part_encoded)
                # Debug statement: script_file.write(f"echo \"curl {download_url} {output_redirector} spdx/{license_id}\"\n")
                script_file.write(f"curl {download_url} {output_redirector} \"spdx/{license_id}\"\n")
                output_redirector = ">>"
